...
# MUST verify to populate failed_results
requests.post(f"{API_URL}/content/verify", json={"generation_id": gen_id})

reg_res = requests.post(f"{API_URL}/content/regenerate", json={"generation_id": gen_id})
print("REGENERATE HTTP STATUS:", reg_res.status_code)
